...
IMAGE_LABELS_DIR = '/goshposh/Multi-label-Inception-net/image_labels_dir/'
IMAGE_DESC_DIR = '/goshposh/Multi-label-Inception-net/image_text_dir/'
...
def get_random_cached_bottlenecks(sess, image_lists, how_many, category,
    ...
    for unused_i in range(how_many):
        ...
        description_file = get_image_labels_path(image_lists, label_name, image_index, IMAGE_DESC_DIR, category)
    ...
    return bottlenecks, ground_truths, descriptions
...
def main(_):
    ...
    file_from_server = '/goshposh/Multi-label-Inception-net/test_with_urls.csv'
    img_labels_path = '/goshposh/Multi-label-Inception-net/image_labels_dir/'
    img_text_path = '/goshposh/Multi-label-Inception-net/image_text_dir/'
    images_path = '/goshposh/Multi-label-Inception-net/images/'

    start = datetime.now()
    if os.path.isfile(file_from_server):
        with open(file_from_server, 'rt') as csvfile:
            input_data = csv.reader(csvfile, delimiter='\t')
            for row in input_data:  # id, url , label
                filename = row[1].split('/')[-1]
                if not os.path.exists(images_path + 'multi-label/' + filename):
                    cmd = ['wget', '-P', images_path + 'multi-label/', row[1]]
                    p = Popen(cmd, shell=False,
                              stdin=None, stdout=None, stderr=None, close_fds=True)
                label_file = open(img_labels_path + filename + '.txt', 'w')

                txt_file = open(img_text_path + filename + '.txt', 'w')
                txt_file.write(clean_str(row[3]))     #clean string and write to file
    ...
    print("Time taken to setup initial files is {} minutes".format((done - start).total_seconds() / 60))

    # Look at the folder structure, and create lists of all the images.
    image_lists = create_image_lists(images_path, FLAGS.testing_percentage,
    ...
    if len(image_lists.keys()) == 0:
        print('Folder containing training images has not been found inside {} directory. \n'
              ...
              .format(images_path, images_path, images_path))
        return -1
    ...
    if do_distort_images:
        ...
        # cached them on disk.
        cache_bottlenecks(sess, image_lists, images_path, FLAGS.bottleneck_dir,
                          jpeg_data_tensor, bottleneck_tensor)
    ...
    for i in range(FLAGS.how_many_training_steps):
        ...
        if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
            ...
            print('%s: Step %d: Cross entropy = %f' % (datetime.now(), i,
                                                       cross_entropy_value))
            validation_bottlenecks, validation_ground_truth, descriptions = (
    ...
    test_bottlenecks, test_ground_truth, descriptions = get_random_cached_bottlenecks(
